...
Use `search`, `sem_search`, and `read` tools to examine the codebase. Use `scout` if deeper research is required for the use-case. Explicitly cite sources using `filepath:line` format in your plan.
...
```bash
./.notagent/skills/create-plan/validate-plan.sh plans/{YYYY-MM-DD}-{task-name}-v{N}.md
```
...
- **ALWAYS validate the plan** using `./.notagent/skills/create-plan/validate-plan.sh` after creation
...
- Use `filepath:line` format for file references (e.g., `crates/notagent_repo/src/provider.rs:45`)
