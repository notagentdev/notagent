...
| 2026-08-13 | `test/terminal.test.ts` | 300 | Task 4 |
...
| 2026-08-13 | `src/autocomplete.ts` | 786 | Task 13 (vorgezogen, weil `editor.ts` aus Task 10 den Provider braucht) |
| 2026-08-13 | `test/autocomplete.test.ts` | 542 | Task 13 (vorgezogen) |
| 2026-08-13 | `src/components/editor.ts` | 2363 | Task 10 |
| 2026-08-13 | `src/editor-component.ts` | 74 | Task 10 |
| 2026-08-13 | `test/editor.test.ts` | 4152 | Task 10 |
| 2026-08-13 | `test/editor-history-keybindings.test.ts` | 43 | Task 10 |
| 2026-08-13 | `src/components/markdown.ts` | 1010 | Task 11 |
| 2026-08-13 | `src/latex.ts` | 1380 | Task 11 |
| 2026-08-13 | `test/markdown.test.ts` | 1667 | Task 11 |
| 2026-08-13 | `test/latex.test.ts` | 496 | Task 11 |
| 2026-08-13 | `test/terminal-image.test.ts` | 632 | Task 12 |
...
| `src/terminal.ts` | 559 | `src/terminal.rs` | verifiziert | Klasse 1: Node-Event-Loop → `pump()` mit tokio-`select!` (stdin-Leser als eigener Thread mit Kanal, Handler laufen auf dem TUI-Strang); `process.stdout.write`-Monkey-Patching der Tests → injizierbare Ausgabe-Senke; `setTimeout`-Timer → Deadlines in `pump()`. Klasse 3: `setRawMode` → termios via libc (Flags empirisch gegen Nodes/libuv Raw-Mode auf macOS verifiziert: `~(BRKINT\|ICRNL\|INPCK\|ISTRIP\|IXON)`, `OPOST\|ONLCR` bleiben, CS8 ohne CSIZE/PARENB, `~(ECHO\|ICANON\|IEXTEN\|ISIG)`, VMIN=1, VTIME=0); `process.stdout.on("resize")` → SIGWINCH via tokio-signal; Windows-VT-Input als direkter Console-API-Aufruf statt Node-Addon. Klasse 1 (Pump-Seam, C-14): `ProcessTerminal::into_shared()` liefert `SharedProcessTerminal` (der `Terminal`-Handle der TUI) plus `ProcessTerminalPump`; die Handler-Aufrufe eines stdin-Chunks werden gesammelt und nach Freigabe des Borrows in unveränderter Reihenfolge zugestellt, weil ein Rust-Handler über die TUI ins Terminal zurückschreibt (TS ruft sie inline, `src/terminal.ts:214-233`); beobachtbar nur bei der Verschränkung mit den Negotiation-Writes desselben Chunks. stdin-Kanal und SIGWINCH-Stream werden für die Wartezeit als Lease ausgeliehen (Rückgabe im `Drop`, damit `select!`-Abbrüche nichts verlieren); Default-Parameter von `drainInput` → `Option<u64>`; `async`-Methode via `async-trait` (dyn-Kompatibilität) |
...
| `src/kill-ring.ts` | 46 | `src/kill_ring.rs` | verifiziert | Keine eigene TS-Suite; über die 22 Kill-/Yank-Fälle von `tests/editor.rs` mitgeprüft (Task 10 vorgezogen) |
| `src/undo-stack.ts` | 28 | `src/undo_stack.rs` | verifiziert | Keine eigene TS-Suite; über die 25 Undo-/Redo-Fälle von `tests/editor.rs` mitgeprüft (Task 10 vorgezogen). Klasse 1: `structuredClone` beim Push → Wertübergabe (für die verwendeten Zustands-Structs gleichbedeutend) |
...
| — (zusätzlich) | — | `tests/pump_loop.rs` (6 Fälle) | Pump-Seam aus C-14 — ohne TS-Vorlage, weil Node den Event-Loop stellt: Dispatch mit rückschreibendem Handler, Lease-Rückgabe nach `select!`-Abbruch, EOF ohne stdin-Kanal, Parken und Wecken von `wait_until_render_due` und die vollständige Dialogschleife `run_until` gegen `startup-ui.ts:74-90` |
