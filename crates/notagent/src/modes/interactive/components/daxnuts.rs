...
/// The animation frame interval; `setInterval(..., 80)`.
const FRAME_INTERVAL: Duration = Duration::from_millis(80);

fn dax_hex() -> String {
    DAX_HEX_PARTS.concat()
}
...
fn parse_image() -> Vec<Vec<[u8; 3]>> {
    ...
    let component = |index: usize| -> u8 {
        let text = std::str::from_utf8(&bytes[index..index + 2]).unwrap_or("00");
    ...
    (0..HEIGHT)
        .map(|y| {
            (0..WIDTH)
                .map(|x| {
                    let index = (y * WIDTH + x) * 6;
                    [component(index), component(index + 2), component(index + 4)]
                })
                .collect()
        })
        .collect()
...
fn build_image() -> Vec<String> {
    ...
    while row < HEIGHT {
        ...
        for x in 0..WIDTH {
            ...
            let bottom = pixels.get(row + 1).map_or(top, |next| next[x]);
            line += &rgb(bottom[0], bottom[1], bottom[2], false);
            line += &rgb(top[0], top[1], top[2], true);
            line.push('▄');
        }
        line += RESET;
        lines.push(line);
...
impl DaxnutsComponent {
    /// New component; the animation starts right away.
    pub fn new() -> Self {
        Self {
            image: build_image(),
            next_frame: Some(Instant::now() + FRAME_INTERVAL),
            tick: 0,
            // ~2 seconds at 80ms
            max_ticks: 25,
    ...
    pub fn tick(&mut self) -> bool {
        ...
        if self.tick >= self.max_ticks {
            ...
            self.next_frame = Some(next_frame + FRAME_INTERVAL);
        }
        self.cached_width = 0;
        true
    }
...
/// `s.replace(/\x1b\[[0-9;]*m/g, "").length` — SGR sequences do not count.
fn plain_length(text: &str) -> usize {
        ...
        if character == '\x1b' && chars.peek() == Some(&'[') {
            ...
            for next in chars.by_ref() {
                ...
                if !next.is_ascii_digit() && next != ';' {
                    // Not an SGR sequence; the regex would not match it.
                    length += 2 + sequence.chars().count() + 1;
                    terminated = true;
                    break;
                }
                sequence.push(next);
...
impl Component for DaxnutsComponent {
    ...
    fn render(&mut self, width: usize) -> Vec<Line> {
        ...
        let revealed_rows = self.image.len().min(
            ((self.tick as f64 / self.max_ticks as f64) * (self.image.len() + 3) as f64).floor()
                as usize,
        );
        ...
        for (index, row) in self.image.iter().enumerate() {
            if index < revealed_rows {
                ...
                lines.push(center(&(rgb(100, 200, 255, false) + &scanline + RESET)));
            } else {
        ...
        let text_phase = self.tick as f64 - self.max_ticks as f64 * 0.6;
        if text_phase > 0.0 || self.tick >= self.max_ticks {
            lines.push(center(
                &theme_instance.fg(ThemeColor::Accent, "Free Kimi K2.5 via OpenCode Zen"),
            ));
            lines.push(center(
                &theme_instance.fg(ThemeColor::Success, "\"Powered by daxnuts\""),
            ));
            lines.push(center(&theme_instance.fg(ThemeColor::Muted, "— @thdxr")));
        } else {
            lines.push(String::new());
            lines.push(String::new());
            lines.push(String::new());
        ...
        lines.push(String::new());
        if text_phase > 2.0 || self.tick >= self.max_ticks {
            lines.push(center(&theme_instance.fg(ThemeColor::Dim, "Try OpenCode")));
            lines.push(center(&theme_instance.fg(
                ThemeColor::MdLink,
