...
use super::canvas::{
    CONT, Canvas, D, L, R, STY_DOT, STY_SOLID, STY_THICK, U, draw_text, draw_text_over_edges,
};
use super::graph::{ClassInfo, Dir, Edge, Graph, Head, LineKind, Node, Shape};
...
/// Placement of one node on the canvas.
...
pub fn order_ranks(by_rank: &mut [Vec<usize>], edges: &[Edge], ranks: &[usize]) {
    ...
    for it in 0..8 {
        ...
        for r in order {
            ...
            for (i, &v) in by_rank[r].iter().enumerate() {
                pos[v] = i;
            }
        }
...
fn sort_by_barycenter(row: &mut [usize], neigh: &[Vec<usize>], pos: &[usize]) {
    let mut keyed: Vec<(f64, usize)> = row
        ...
        .collect();
    // `Array.prototype.sort` is stable, `sort_by` is too.
    keyed.sort_by(|a, b| a.0.partial_cmp(&b.0).expect("barycenters are finite"));
    for (slot, (_, v)) in row.iter_mut().zip(keyed) {
        *slot = v;
    }
}

/// Count how many pairs of adjacent-rank edges cross.
...
pub fn assign_positions(
    ...
    for it in 0..10 {
        let rows: Vec<&Vec<usize>> = if it % 2 == 0 {
            by_rank.iter().collect()
        } else {
            by_rank.iter().rev().collect()
        };
        let neigh = if it % 2 == 0 { &parents } else { &children };
        for row in rows {
    ...
    (0..n)
        .map(|v| js_round(pos[v] - min_left).max(0.0) as usize)
        .collect()
}

/// `Math.round`: halves go up, unlike Rust's round-half-away-from-zero.
fn js_round(value: f64) -> f64 {
    (value + 0.5).floor()
}

fn relax_rank(nodes: &[usize], neigh: &[Vec<usize>], pos: &mut [f64], size: &[usize], sep: f64) {
    let n = nodes.len();
    if n == 0 {
        return;
    }
...
pub fn assign_tracks(spans: &[Span5]) -> (Vec<(usize, usize)>, usize) {
    let mut sorted = spans.to_vec();
    sorted.sort();
    let mut tracks: Vec<Vec<[usize; 4]>> = Vec::new();
...
fn place_td(
    ...
    for (r, row) in by_rank.iter().enumerate() {
        for &idx in row {
            ...
            let cx = centers[idx];
            let x = sat(cx, half(w));
            let y = rank_y[r] + half(sat(rank_h[r], h + sizes.extra_h[idx]));
            placed[idx] = Placed {
                x,
                y,
                w,
...
fn place_lr(
    ...
    let self_tails = by_rank[max_rank]
        .iter()
        .filter(|&&i| sizes.extra_h[i] > 0 && sizes.self_label_w[i] > 0)
        .map(|&i| 2 + sizes.self_label_w[i])
        .max()
        .unwrap_or(0);
    let canvas_w = rank_x[max_rank] + col_w[max_rank] + self_tails;
...
pub fn layout_canvas(graph: &Graph, extras: &[NodeExtra]) -> CanvasResult {
    ...
    let box_h: Vec<usize> = extras
        ...
        .map(|(i, extra)| match extra {
            ...
            NodeExtra::Compartments(sections) => {
                ...
                sections.iter().map(Vec::len).sum::<usize>() + sat(filled, 1) + 2
            }
    ...
    for e in &graph.edges {
        ...
        extra_h[e.from] = 2;
        if let Some(label) = e.label.as_deref() {
    ...
    let vertical = graph.dir == Dir::Down || graph.dir == Dir::Up;
...
pub fn layout_class(graph: &Graph, infos: &[ClassInfo]) -> CanvasResult {
    let extras: Vec<NodeExtra> = graph
        ...
        .map(|(i, node)| {
            ...
            if let Some(annotation) = infos[i].annotation.as_deref() {
...
pub fn layout_grouped(graph: &Graph) -> CanvasResult {
    ...
    let endpoint = |n: usize| -> (ItemKey, Vec<usize>) {
        match proxy.get(&n) {
            None => (ItemKey::Node(n), group_chain(graph.node_group[n])),
            Some(&gi) => (ItemKey::Group(gi), group_chain(graph.groups[gi].parent)),
        }
    };
    ...
    for (ei, e) in graph.edges.iter().enumerate() {
        ...
        let t = endpoint(e.to);
        let mut k = 0;
        while k < f.1.len() && k < t.1.len() && f.1[k] == t.1[k] {
            k += 1;
        }
        let scope = if k == 0 { None } else { Some(f.1[k - 1]) };
        let f_key = if f.1.len() > k {
            ItemKey::Group(f.1[k])
        } else {
            f.0
        };
        let t_key = if t.1.len() > k {
            ItemKey::Group(t.1[k])
        } else {
            t.0
        };
    ...
    build_scope(graph, None, &scope_edges, &direct_nodes, &keep).map(|canvas| orient(canvas, graph))
}
...
fn build_scope(
    ...
    let mut items: Vec<ItemKey> = direct_nodes
        .get(&scope)
        .map(|list| list.iter().map(|&n| ItemKey::Node(n)).collect())
        .unwrap_or_default();
    let child_groups: Vec<usize> = (0..graph.groups.len())
        .filter(|&gi| graph.groups[gi].parent == scope && keep[gi])
        .collect();
    items.extend(child_groups.iter().map(|&gi| ItemKey::Group(gi)));
    ...
    for &(f, t, ei) in scope_edges.get(&scope).into_iter().flatten() {
...
/// Draw a node box with its wrapped label.
pub fn draw_box(canvas: &mut Canvas, p: &Placed, lines: &[String], shape: Shape) {
    let Placed { x, y, w, h, .. } = *p;
    let right = x + w - 1;
    let bottom = y + h - 1;

    let rounded = shape == Shape::Round || shape == Shape::Diamond;
...
fn draw_frame(canvas: &mut Canvas, p: &Placed, title: &str, sub: &Canvas) {
    ...
    canvas.blit(
        sub,
        p.x + 1 + half(sat(p.w - 2, sub.w)),
        p.y + 1 + half(sat(p.h - 2, sub.h)),
    );
}
