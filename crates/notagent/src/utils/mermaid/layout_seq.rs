...
use super::layout::{CanvasResult, Placed, draw_box};
...
/// `Math.ceil(n / 2)` on non-negative integers.
fn half_up(n: usize) -> usize {
    n.div_ceil(2)
}
...
fn note_geometry(xs: &[usize], anchor: NoteAnchor, text_w: usize) -> (usize, usize) {
    match anchor {
        NoteAnchor::Over { from, to } => {
            ...
            let w = (sat(xs[to], xs[from]) + 5).max(text_w + 2 * PAD + 2);
            (sat(center, half(w)), w)
        }
        NoteAnchor::Left { at } => {
...
fn item_text_w(text: Option<&str>) -> usize {
    text.map_or(0, string_width)
}

/// Geometry for a box drawn by position and size; ranks are irrelevant here.
fn seq_box(x: usize, y: usize, w: usize, h: usize) -> Placed {
    Placed {
        x,
        y,
        w,
...
pub fn layout_sequence(seq: &Sequence) -> CanvasResult {
    ...
    let mut gaps: Vec<usize> = (0..sat(n, 1))
        .map(|i| SEQ_GAP.max(half_up(box_w[i]) + half_up(box_w[i + 1]) + 1))
        .collect();
    ...
    for item in &seq.items {
        match item {
            SeqItem::Message { from, to, text, .. } => {
                let tw = item_text_w(text.as_deref());
                if from != to {
    ...
    let mut canvas_w = xs[n - 1] + half_up(box_w[n - 1]) + 1;
    for item in &seq.items {
        match item {
            SeqItem::Message { from, to, text, .. } if from == to => {
                canvas_w = canvas_w.max(xs[*from] + 5 + item_text_w(text.as_deref()) + 1);
            }
            SeqItem::Note { anchor, text } => {
                let (x, w) = note_geometry(&xs, *anchor, string_width(text));
    ...
    for i in 0..n {
        for by in [0, bottom_top] {
            draw_box(
                &mut canvas,
                &seq_box(sat(xs[i], half(box_w[i])), by, box_w[i], box_h),
    ...
    for (k, item) in seq.items.iter().enumerate() {
        let SeqItem::Note { anchor, text } = item else {
            continue;
        };
        let (x, w) = note_geometry(&xs, *anchor, string_width(text));
        draw_box(
            &mut canvas,
            &seq_box(x, rows[k], w, 3),
    ...
    for &x in &xs {
        ...
        canvas.seg_v(x, box_h, sat(bottom_top, 1));
...
fn draw_message(canvas: &mut Canvas, item: &SeqItem, xs: &[usize], r: usize) {
    ...
    canvas.set(
        if rightward { sat(x1, 1) } else { x1 + 1 },
        arrow_row,
        head_ch,
        Cls::Edge,
    );
