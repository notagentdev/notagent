//! `togetherProvider()`.
//!
//! 1:1 port of `packages/ai/src/providers/together.ts` (15 LOC). The generated
//! `together.models.ts` is the catalog snapshot in `data/`, read through
//! [`get_builtin_models`].

use std::sync::Arc;

use crate::api::streams::OpenAICompletionsApi;
use crate::auth::helpers::EnvApiKeyAuth;
use crate::auth::types::ProviderAuth;
use crate::model_catalog::get_builtin_models;
use crate::models::{BuiltProvider, CreateProviderOptions, ProviderApis, create_provider};

/// `togetherProvider()`
pub fn together_provider() -> Arc<BuiltProvider> {
    create_provider(CreateProviderOptions {
        id: "together".to_string(),
        name: Some("Together".to_string()),
        base_url: Some("https://api.together.ai/v1".to_string()),
        headers: None,
        auth: ProviderAuth {
            api_key: Some(Arc::new(EnvApiKeyAuth::new(
                "Together API key",
                ["TOGETHER_API_KEY"],
            ))),
            oauth: None,
        },
        models: get_builtin_models("together"),
        fetch_models: None,
        filter_models: None,
        api: ProviderApis::Single(Arc::new(OpenAICompletionsApi)),
    })
}
