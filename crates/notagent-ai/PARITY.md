...
| `src/utils/retry.ts` | 228 | `utils/retry.rs` | verifiziert (Task 3) | Klasse 3: `AbortSignal` → `CancellationToken`; Klasse 1: Callbacks als `Arc<dyn Fn>`. **Härtung (v0.1.4):** Der exponentielle Backoff saturiert (`saturating_mul`/`saturating_pow`) statt bei `retry.n` ≥ 65 im Debug zu panicken bzw. im Release zu wrappen; TS wächst dort als Float einfach weiter |
| `src/utils/provider-retry.ts` | 125 | `utils/provider_retry.rs` | verifiziert (Task 3) | Klasse 3: Die SDK-Fehlerform wird zum Trait `ProviderErrorInfo`, das die HTTP-Schicht implementiert. **Bewusste Abweichung (Nutzerentscheid 2026-08-16, v0.1.4):** Ein nicht parsebarer `Retry-After`-Header fällt in den exponentiellen Backoff, statt wie in TS als NaN die Validierung zu passieren und 0 ms zu schlafen (Retry ohne jedes Backoff) |
...
| `src/utils/node-http-proxy.ts` | 112 | `utils/node_http_proxy.rs` | verifiziert (Task 3) | Klasse 3: liefert die Proxy-URL für reqwest statt eines undici-Agents. **Paritäts-Bugfix (v0.1.4):** Der Minimal-Parser schreibt Schema und Hostname jetzt klein wie der WHATWG-`URL`-Parser der TS-Seite — vorher umging `HTTPS://EXAMPLE.COM` die `no_proxy`-Liste (Test: `no_proxy_matches_case_insensitively_like_the_whatwg_parser`) |
...
| `src/api/openrouter-images.ts` | 196 | `api/openrouter_images.rs` | verifiziert (Task 10) | Klasse 3: Der `openai`-SDK-Client wird durch einen direkten POST auf `<baseUrl>/chat/completions` ersetzt — genau die Anfrage, die das SDK stellt. **Bugfix (v0.1.4):** `read_body` reicht Streamfehler als `Err` durch statt sie zu verschlucken — ein mittendrin abgebrochener Body erschien sonst als irreführender JSON-Parse-Fehler; das SDK der TS-Seite rejected dort ebenfalls |
