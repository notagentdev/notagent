...
| src/server.ts | 396 | src/server.rs | verifiziert | Klasse 1: `void promise`-Fire-and-forget → `tokio::spawn`; Handshake-Timeout als abbrechbarer Task; Stage-Automat und Fehler-Sanitizing 1:1. **Härtung (v0.1.4):** `start()` belegt das `starting`-Flag per `compare_exchange` — die Atomarität, die TS gratis aus der Single-Thread-Eventloop bezieht, hatte die Load-dann-Store-Übersetzung verloren (zwei parallele `start()` konnten beide passieren) |
